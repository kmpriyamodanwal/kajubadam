...
// $order_detail->finalprice = 1;


?>

	<img src="<?=base_url('payment-animation.gif') ?>" class="img1111">

	<form method="POST" name="customerData" action="requestHandler" style="display: none;">
		<table width="40%" height="100" border='1' align="center"><caption><font size="4" color="blue"><b>Integration Kit</b></font></caption>
			<table width="50%" height="100" border='1' align="center">
					...
					TID	:<input type="text" name="tid" value="<?=$order_detail->order_id ?>" id="tid" readonly />
				
				
					Merchant Id	:<input type="text" name="merchant_id" value="<?=ccavanue_merchant_id ?>"/>
				
				
					Order Id	:<input type="text" name="order_id" value="<?=$order_detail->order_id ?>"/>
				
				
					Amount	:<input type="text" name="amount" value="<?=$order_detail->finalprice ?>"/>
				
				
					Currency	:<input type="text" name="currency" value="INR"/>
				
				
					Redirect URL	:<input type="text" name="redirect_url" value="<?=base_url('ccavanue/response?shopping_order_id='.$order_detail->id) ?>" /> 
					
				
			 	
			 		Cancel URL	:<input type="text" name="cancel_url" value="<?=base_url('ccavanue/response?shopping_order_id='.$order_detail->id) ?>"/>			 		
		        	...
		        	Billing Name	:<input type="text" name="billing_name" value="<?=$order_detail->f_name ?>"/>		        
		        	Billing Address	:<input type="text" name="billing_address" value="<?=$order_detail->address ?>"/>		        
		        	Billing City	:<input type="text" name="billing_city" value="<?=$state?>"/>		        
		        	Billing State	:<input type="text" name="billing_state" value="<?=$state ?>"/>		        
		        	Billing Zip	:<input type="text" name="billing_zip" value="110086"/>		        
		        	Billing Country	:<input type="text" name="billing_country" value="India"/>		        
		        	Billing Tel	:<input type="text" name="billing_tel" value="<?=$order_detail->mobile ?>"/>		        
		        	Billing Email	:<input type="text" name="billing_email" value="<?=$order_detail->email ?>"/>
		        	...
		        	Shipping Name	:<input type="text" name="delivery_name" value="<?=$order_detail->f_name ?>"/>		        
		        	Shipping Address	:<input type="text" name="delivery_address" value="<?=$order_detail->address ?>"/>		        
		        	shipping City	:<input type="text" name="delivery_city" value="<?=$state ?>"/>		        
		        	shipping State	:<input type="text" name="delivery_state" value="<?=$state ?>"/>		        
		        	shipping Zip	:<input type="text" name="delivery_zip" value="110086"/>		        
		        	shipping Country	:<input type="text" name="delivery_country" value="India"/>		        
		        	Shipping Tel	:<input type="text" name="delivery_tel" value="<?=$order_detail->mobile ?>">

		        	Merchant Param1	:<input type="text" name="merchant_param1" value="additional Info."/>
		        
		        
		        	Merchant Param2	:<input type="text" name="merchant_param2" value="additional Info."/>
		        
				
					Merchant Param3	:<input type="text" name="merchant_param3" value="additional Info."/>
				
				
					Merchant Param4	:<input type="text" name="merchant_param4" value="additional Info."/>
				
				
					Merchant Param5	:<input type="text" name="merchant_param5" value="additional Info."/>
		        ...
		        <!-- EMI section end -->
				 ...
				 Promotions:
		            	 <select name="promo_code" id="promo_code"> <option value="">All Promotions &amp; Offers</option> </select> 
           	...
           	$.each(jsonData, function(index,value) {
	              ...
	              if(paymentOption =="OPTEMI"){
		              if(value.payOpt=="OPTEMI"){
	                	...
	                	emiPlanTr="&nbsp;EMI PlanMonthly InstallmentsTotal Cost";
							
	                	$.each(emiPlansArray, function() {
		                	emiPlanTr=emiPlanTr+
							"<tr class='tenuremonth "+this['planId']+"' id='"+this['tenureId']+"' style='display: none'>"+
								" <input type='radio' name='emi_plan_radio' id='"+this['tenureMonths']+"' value='"+this['tenureId']+"' class='emi_plan_radio' > "+
								""+this['tenureMonths']+ "EMIs. <label class='merchant_subvention'>@ <label class='emi_processing_fee_percent'>"+this['processingFeePercent']+"</label>&nbsp;%p.a</label>"+
								""+
								""+this['currency']+"&nbsp;"+this['emiAmount'].toFixed(2)+
								""+
								"<label class='currency'>"+this['currency']+"</label>&nbsp;"+ 
